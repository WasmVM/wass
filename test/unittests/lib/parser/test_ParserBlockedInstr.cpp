...
TEST(unittest_ParserBlockedInstr, block){
  ...
  EXPECT_STREQ(instr->label->c_str(), "test");
  EXPECT_TRUE(std::holds_alternative<ValueType>(*(instr->blockType)));
  EXPECT_EQ(std::get<ValueType>(*(instr->blockType)), ValueType::f32);
...
TEST(unittest_ParserBlockedInstr, block_with_postfix){
  ...
  BlockInstr* instr = std::get_if<BlockInstr>(&result);
  EXPECT_STREQ(instr->label->c_str(), "test");
  EXPECT_TRUE(std::holds_alternative<TypeUse>(*(instr->blockType)));
