...
TEST(unittest_GenElement, one_index){
  ...
  EXPECT_EQ(std::visit<BinaryCode>(visitor, CodeGenVariant(data)), BinaryCode({'\x02', '\x41', '\x07', '\x0B', '\x01', '\x03'}));
}
...
TEST(unittest_GenElement, two_indices){
  ...
  CodeGenVisitor visitor;
  EXPECT_EQ(std::visit<BinaryCode>(visitor, CodeGenVariant(data)), BinaryCode({'\x02', '\x41', '\x07', '\x0B', '\x02', '\x03', '\x06'}));
}